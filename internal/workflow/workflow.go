// Package workflow builds the deterministic handoff artifact for the top-level
// `faultline workflow` command. It does not execute remediation steps.
package workflow
...
// Mode selects the output shape for workflow handoff generation.
type Mode string
...
// Plan is the deterministic next-step handoff derived from the top diagnosis.
type Plan struct {
