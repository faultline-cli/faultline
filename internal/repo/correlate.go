...
func failurePatterns(category, playbookID string) []string {
	...
	case "missing-env", "config-mismatch":
	...
	switch category {
	case "auth":
		return append(commonCI, ".env*", "deploy*", "Dockerfile", "*.yaml", "*.yml")
	case "build":
		return append(commonCI, "Dockerfile", "go.mod", "go.sum", "package.json", "package-lock.json", "yarn.lock", "pnpm-lock.yaml", "pyproject.toml", "Gemfile", "requirements*.txt")
	case "deploy":
		return append(commonCI, "deploy*", "k8s*", "kubernetes*", "helm*", "*.yaml", "*.yml", "Dockerfile")
	case "test":
		return []string{"*_test.go", "testdata*", "fixtures*", "migrations*", "*.sql", "db*", "internal/*test*", "pkg/*test*"}
	case "runtime":
