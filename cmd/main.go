...
//
//	0 — success / no findings (batch: all logs matched)
//	1 — operational finding (batch: one or more unmatched; analyze: silent failure with --fail-on-silent)
...
func main() {
	if err := newRootCommand().Execute(); err != nil {
		switch {
		case errors.Is(err, app.ErrSilentFailure),
