...
- [Goroutine spawned without context or stop channel](../runtime/goroutine-leak.md) — A goroutine is spawned with an infinite loop and no context cancellation, stop channel, or WaitGroup — it will leak
- [HTTP client used without a timeout](../runtime/http-client-no-timeout.md) — An HTTP request is made using the package-level `http.
- [Error return value ignored or silently discarded](../runtime/missing-error-propagation.md) — An error return from an I/O, database, or side-effecting call is discarded with `_ =` or silently overwritten, so failures pass undetected.
- [Database transaction opened without a deferred rollback](../runtime/missing-transaction-rollback.md) — A database transaction is opened and committed in the same function with no
`defer tx.
...
- [Unrecovered panic in HTTP or RPC handler](../runtime/panic-in-http-handler.md) — A `panic` call appears inside an HTTP or RPC handler with no `recover()` in a deferred function, which can crash
...
- [TLS certificate verification disabled in HTTP client](../security/insecure-tls-skip-verify.md) — TLS certificate verification is explicitly disabled, allowing the client to connect to servers with invalid, expired, or self-signed certificates without
...
- [Hardcoded secret, token, or password in source code](../silent-failure/hardcoded-secret.md) — A secret-named variable is assigned a literal string value in source code, exposing credentials, API keys, or tokens to anyone with repository access.
